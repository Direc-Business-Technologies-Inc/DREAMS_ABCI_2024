...
namespace ABROWN_DREAMS
{
    public partial class Projects : Page
    {
        ...
        {
            gvProjectList.DataSource = ws.GetProjects();
        ...
        void LoadBlockList()
        {
            ...
            gvSAPBlocks.DataSource = ws.GetBlockList(txtProjId.Value);
            gvSAPBlocks.DataBind();

            gvBLockList.DataSource = ws.GetBlockByProject(txtProjId.Value);
        ...
        protected void btnYes_Click(object sender, EventArgs e)
        {
            ...
            else if ((string)Session["ConfirmType"] == "removeLot")
            {
                if (ws.DeleteLot(txtProjId.Value, txtBlockLot.Value, (string)Session["SelectedLot"]) == true)
                {
                    //successful
                    alertMsg("Lot removed successfully", "success");
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "", "LotAllocationTab();", true);
                }
                else
                {
                    closeconfirm();
                    alertMsg("Error in removing selected lot", "error");
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "", "LotAllocationTab();", true);
                }
            }
            ...
            else if ((string)Session["ConfirmType"] == "removeblocklocation")
            {
                ...
                if (hana.Execute($@"UPDATE ""PRJ1"" SET ""left"" = NULL,""top"" = NULL,""type"" = NULL  Where ""Block"" ='{txtSelectedProjectBlock.Value}' and ""PrjCode"" = '{txtProjId.Value}'", hana.GetConnection("SAOHana")))
                {
                    LoadBlockList();
                    closeconfirm();
            ...
            else if ((string)Session["ConfirmType"] == "removelotlocation")
            {
                //** delete lot location
                if (hana.Execute($@"UPDATE ""PRJ1"" SET ""left"" = NULL,""top"" = NULL,""type"" = NULL  Where ""Block"" ='{txtSelectedProjectBlock.Value}' and ""PrjCode"" = '{txtProjId.Value}'", hana.GetConnection("SAOHana")))
                {
                    hana.Execute($@"UPDATE ""PRJ2"" SET ""type"" = NULL,""left"" = NULL,""top"" = NULL Where ""PrjCode"" = '{txtProjId.Value}' and ""Block""='{txtBlockLot.Value}' and ""Lot""='{txtLot.Text}'", hana.GetConnection("SAOHana"));
        ...
        protected void gvBLockList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("Select"))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    imgBlockWidth.Value = width;
                    imgBlockHeight.Value = height;

                    //delete tmp lot
                    //DeleteLotTemp(projCode, Block);
                    //bind lot
                    GetLot(projCode, Block);
                    //close modal
                ...
                else
                {
                    //close modal
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "", "closeBlockList();", true);
                    alertMsg("Selected block has no image file", "error");
                }
            }
            ReloadCanvas();
        }
        ...
        void GetLot(string proj, string block)
        {
            gvSAPLot.DataSource = ws.GetLotList(proj, block);
            gvSAPLot.DataBind();
        }
        ...
        {
            if (!string.IsNullOrEmpty(txtSelectedProjectBlock.Value))
            {
                DataTable dt = hana.GetData($@"SELECT * FROM ""PRJ1"" Where ""PrjCode"" = '{txtProjId.Value}' and ""Block"" = '{txtSelectedProjectBlock.Value}'", hana.GetConnection("SAOHana"));
                if (dt.Rows.Count > 0)
                {
                    if (ws.UpdateBlockLocation(txtProjId.Value, txtSelectedProjectBlock.Value, Convert.ToInt32(txtPixelX.Text), Convert.ToInt32(txtPixelY.Text)))
                    {
                        LoadBlockList();
                ...
                {
                    alertMsg($"Please upload image for Block: {txtSelectedProjectBlock.Value}", "warning");
                    LoadBlockList();
                    txtSelectedProjectBlock.Value = string.Empty;
                }
            }
        ...
        protected void gvSAPBlocks_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //Check if Block has image
                if (!string.IsNullOrEmpty(e.Row.Cells[3].Text.Trim()))
                {
                    img.ImageUrl = "~/assets/img/checked.png";
                }
                else
