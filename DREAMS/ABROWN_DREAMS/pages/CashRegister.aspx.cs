...
namespace ABROWN_DREAMS
{
	public partial class CashRegister : Page
	{
													...
													else
													{
														...
														else
														{
															...
															else
															{
																...
																foreach (GridViewRow _row in gvDownPayments.Rows)
																{
																	...
																	if (chkSel.Checked)
																	{
																		...
																		if (paymentTerm == 2 && paymentdue_type == "DP")
																		{
																			string test1 = "";
																		}
																		...
																		else
																		{
																			...
																			if (paymentType == "LB" && paymentTerm == 2)
																			{
																				string test = "";
																			}
																			...
																			if (Convert.ToInt32(ViewState["chkTagMISC"]) < 1)
																			{
																				...
																				if (!DataHelper.DataTableExist(dtCheckARResInvoice2))
																				{
																					...
																					if (_dtARPartialPaidJE2.Rows.Count > 0)
																					{
																						...
																						cashRegister.CreateJournalEntry(null, ProjectCode, SapCardCode, _ARAmountJE2, SODocEntry, FinancingScheme, TaxClassification,
																								   Block, Lot, APOthersAccount, ARClearingAccount, "", "ARDPJE2", txtDocNum.Text, "", "", Vatable, Convert.ToDateTime(tDocDate.Value).ToString("yyyyMMdd"), "RVRS",
																								   out JournalEntryNo, out Message, receiptNo, 0, null, 0, _TransId);
                                                           ...
                                                           where ""U_BlockNo"" = '{Block}' and ""U_LotNo"" = '{Lot}' and ""U_Project"" = '{ProjectCode}'", hana.GetConnection("SAPHana"));
																		...
																		if (((_TCPPayment + currentPayment) >= (double.Parse(lblNETTCP.Text) * Convert.ToDouble(_ARReserveInvoiceCondition1))) && !string.IsNullOrWhiteSpace(_LatestLTSNo))
																		{
																...
																if (isSuccess)
																{
																	...
																	if (isSuccess)
																	{
																		...
																		if (lblApplyToPrincipal.Text == "YES")
																		{
																			...
																			string DPDocEntry = "0";
																			string MiscEntry = "0";
																			Restructuring(out Message, out QuotationDocEntry, out DPDocEntry, out MiscEntry);
																			//alertMsg(Message, "warning");
																		}
			...
			try
			{
				...
				if (ViewState["SQDocEntry"] != null)
				{
					...
					foreach (GridViewRow row in gvDownPayments.Rows)
					{
						...
						double SurCharges = Math.Round(double.Parse(row.Cells[8].Text));
						double IPS1 = Math.Round(double.Parse(row.Cells[9].Text));
						double Principal = Math.Round(double.Parse(row.Cells[6].Text));
						...
						double totalPayment = Math.Round(rowCurrentPayment);
						...
						if (LineStatus == "O") //check if status is open
						{
							if (Type != "MISC") //check if not miscellaneous payment
							{
								...
								if (chkSel.Checked) //check if selected
								{
									...
									double tempValue = totalPayment - (Interest + SurCharges + IPS1);
									if (tempValue > 0)
									{
										tempTCPTotalPayment += tempValue;
									}
			...
			)
		{
			...
			else if (currentPayment < (Math.Round(principal + interest + surcharge + IPS)))
			{
				if (Math.Round(interest + surcharge + IPS) >= Math.Round(previousPayment + currentPayment))
				{
					qut13Amount = 0;
				}
				else if (previousPayment == 0)
				{
					if (currentPayment < (Math.Round(principal + interest + surcharge + IPS)))
					{
						var test = currentPayment;
						var test2 = Math.Round(principal + interest + surcharge + IPS);
						//qut13Amount = currentPayment;
				...
				{
					if ((Math.Round(principal + interest + surcharge + IPS)) <= principal)
					{
						qut13Amount = currentPayment;
					}
					else
					{
						if (((Math.Round(principal + interest + surcharge + IPS, 2)) - (Math.Round(previousPayment + currentPayment))) > principal)
						{
							qut13Amount = currentPayment - ((Math.Round(principal + interest + surcharge + IPS, 2)));
						}
						else
						{
							//2023-10-16 : add amounts for additional checking
							...
							tempAmount = currentPayment - Math.Round((surchargePosted + interestPosted), 2);
							if (tempAmount < 0)
							{
								qut13Amount = 0;
							}
		...
		private string Restructuring(out string errorMsg, out string QuotationDocEntry,
									 out string DPDocEntry, out string MiscEntry)
		{
